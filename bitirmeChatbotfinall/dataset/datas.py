    ...
    for entry in data:
        if entry["type"] == "announcement":
            formatted_data.append({
                "prompt": f"Duyuru: {entry['title']} - {entry['date']}",
                "completion": f"{entry['title']}, {entry['link']}, {entry['date']}, {entry['description']}"
            })
        elif entry["type"] == "event":
            formatted_data.append({
                "prompt": f"Etkinlik: {entry['title']} - {entry['date']}",
                "completion": f"{entry['title']}, {entry['date']}, {entry['time']}, {entry['location']}, {entry['link']}"
            })
        elif entry["type"] == "academic_staff":
            formatted_data.append({
                "prompt": f"Personel: {entry['name']}",
                "completion": f"{entry['mail']}"
            })
...
def save_data_to_jsonl(data, filename="utils/data.jsonl"):
